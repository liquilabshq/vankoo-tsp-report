...
| Repository | Branch | Commit Id | Commit Message | Commit Message Body | Committed on (Date) |
|---|---|---|---|---|---|
| liquilabshq/vankoo-finance-service | develop | a26482d | Merge pull request #25 from liquilabshq/feature/wallet-debit-endpoint | feat(finance): debit endpoint, idempotency and X-User-Id ownership check | 16/09/2026 |
...
| liquilabshq/vankoo-finance-service | develop | 4a233dd | Merge pull request #24 from liquilabshq/docs/finance-stripe-in-compose | docs(finance): how Stripe keys reach the service | 16/09/2026 |
| liquilabshq/vankoo-finance-service | docs/finance-stripe-in-compose | eb3ef51 | docs(finance): how Stripe keys reach the service when it runs in the compose | The guide only covered running from the IDE. Now that finance-service has a container, the keys come from vankoo-infra's .env through environment:, config/application-local.yaml never enters the image, and stripe listen keeps pointing at localhost:8083 because the port is published. | 16/09/2026 |
| liquilabshq/vankoo-finance-service | develop | 063f5a1 | Merge pull request #23 from liquilabshq/feature/finance-dockerfile | chore(finance): add Dockerfile and .dockerignore | 16/09/2026 |
| liquilabshq/vankoo-finance-service | feature/finance-dockerfile | bd1344f | chore(finance): add Dockerfile and .dockerignore | Same two-stage Maven template as iam-service, gateway and discovery (finance-*.jar, port 8083, non-root user, docker profile). The .dockerignore keeps config/ out of the build context: application-local.yaml lives there with the real Stripe keys. | 16/09/2026 |
| liquilabshq/vankoo-finance-service | develop | f71e52b | Merge pull request #22 from liquilabshq/chore/finance-api-prefix | refactor(finance): serve the REST API under /api/v1 like the other services | 15/09/2026 |
| liquilabshq/vankoo-finance-service | chore/finance-api-prefix | 6993e2c | refactor(finance): serve the REST API under /api/v1 like the other services | IAM and Investment mount their controllers under /api/v1; Finance alone used a bare /v1. The gateway hid the difference from clients, but every route definition had to remember which service was the odd one out. Move the two controllers and the Stripe webhook to /api/v1, and update the tests and the docs that spell the paths out (contracts, UML, the Stripe guide).  No behaviour changes: the same handlers answer at the new prefix, and DepositControllerTest and WalletControllerTest pass against it. | 15/09/2026 |
| liquilabshq/vankoo-finance-service | develop | 2c54644 | Merge pull request #21 from liquilabshq/fix/finance-kafka-send-failures | Fix/finance kafka send failures | 05/09/2026 |
| liquilabshq/vankoo-finance-service | fix/finance-kafka-send-failures | dd430ad | docs(finance): record how Kafka send failures are detected | Covers the synchronous publication, the namespace the Kafka producer properties actually bind to, the timeout budget under Axon's handler timeout, and the manual procedure for the half the automated test does not reach. Notes that DepositChargeCreator and WalletCreditor carry the same LoggingErrorHandler problem. | 05/09/2026 |
| liquilabshq/vankoo-finance-service | fix/finance-kafka-send-failures | ac055a8 | test(finance): cover Kafka send failures and a broker outage | KafkaOutageIntegrationTest runs against the dev profile's real binding configuration and pauses a Testcontainers broker mid-flight, which is the card's acceptance criterion. With sync: false and nothing else changed it fails, so it does pin the fix. It needs Docker; the two unit tests do not.  Also drops the duplicate spring-cloud-stream-test-binder declaration Maven warned about on every build. | 05/09/2026 |
| liquilabshq/vankoo-finance-service | fix/finance-kafka-send-failures | 011f013 | fix(finance): detect Kafka send failures in the integration publisher | streamBridge.send() ignored its result and the binding published asynchronously, so a broker rejection reached only a failure channel that is null unless errorChannelEnabled is on: the handler returned cleanly, Axon advanced the deposit-integration-events token and the outcome was gone for good.  The producer block also moves to spring.cloud.stream.kafka.bindings, where the Kafka binder actually reads it. Under spring.cloud.stream.bindings it was ignored, which had been silently disabling key.serializer since the binding was written: verified against a real broker, publishing failed outright with a SerializationException.  Throwing is not enough on its own. Axon's default LoggingErrorHandler swallows the exception and lets the token advance, so this registers a PropagatingErrorHandler for that group only, and brings the producer timeouts under Axon's 30s handler timeout. | 05/09/2026 |
| liquilabshq/vankoo-finance-service | develop | 871e22b | Merge pull request #20 from liquilabshq/feature/finance-deposit-charge-creation | feat(finance): create the Stripe charge when a deposit is initiated | 04/09/2026 |
| liquilabshq/vankoo-finance-service | feature/finance-deposit-charge-creation | 33d1f0f | docs(finance): keep local secrets out of the packaged jar | The guide told you to create application-local.yaml under src/main/resources. Maven copies that directory into target/classes, so the file — and the sk_test_ key in it — gets packaged into the jar. Git never leaks it, but a jar is shared and deployed with far less ceremony than a config file, so the secret travels further than the one place it was meant to stay.  config/ at the project root is one of Spring Boot's default config locations and is never packaged, so the file is still picked up with no extra configuration and stops riding along in build output.  The guide now says why the src/main/resources path is wrong rather than just naming the new one, since it is the kind of mistake that looks harmless. .gitignore drops the two src/main/resources entries: the bare filename patterns already match at any depth, including config/. | 04/09/2026 |
| liquilabshq/vankoo-finance-service | feature/finance-deposit-charge-creation | 0ee5e27 | docs(finance): drop .env.example, Spring Boot never reads it | The .env.example added in b467ca3 promised a mechanism this service does not have. Spring Boot has no native .env support and nothing in the pom.xml adds it, so copying the file to .env did nothing at all: the properties kept their empty defaults and the caller still hit the IllegalStateException the guide describes. A file that looks like configuration and silently is not is worse than no file.  It came from following vankoo-profile-service's convention without checking that the convention travels. That service is NestJS, where dotenv loads .env natively; vankoo-infra's .env is read by Docker Compose. Neither applies to a Spring Boot service.  It was also redundant. application.yaml already declares every property with its ${...} binding and its default, so the list of names lived in two places at once — and the YAML is the one the framework actually reads.  The guide now documents what cannot be inferred from the YAML instead of repeating it: which operation requires which property, the exact exception a missing one throws, where each value comes from in the dashboard, and a new section on how to supply values locally — a gitignored application-local.yaml activated with SPRING_PROFILES_ACTIVE=dev,local, or environment variables on the run configuration, which is the same mechanism a container will use.  .gitignore now ignores application-local.yaml. It keeps ignoring .env too: the framework will not read one, but someone may still create it out of habit and it would hold a live secret key. | 02/09/2026 |
| liquilabshq/vankoo-finance-service | feature/finance-deposit-charge-creation | d268fc0 | feat(finance): create the Stripe charge when a deposit is initiated | The InitiateDepositCommand handler does no I/O — it validates and applies DepositInitiatedEvent — and nothing reacted to that event, so a deposit sat in PENDING forever and never reached Stripe at all.  DepositChargeCreator closes that: it calls PaymentProvider.createDeposit and dispatches RegisterDepositProviderReferenceCommand with the answer. A plain @EventHandler on the mould of WalletCreditor, not a saga — one outbound call and one dispatch, nothing to wait for and nothing to compensate.  It deliberately does not move the deposit's status. createDeposit answers ACTION_REQUIRED, but ApplyProviderDepositUpdateCommand requires a providerEventId that the response does not carry, so only the webhook moves the status and the actionUrl rides on the registration command, which is where the contract already puts it.  Retries are delegated to Axon. A transient failure is rethrown so the processor leaves its token where it was; the IdempotencyKey comes from the event, so every attempt sends the same one and Stripe returns the original session instead of opening a second charge. Retryability is decided through the RetryablePaymentProviderException marker, never by concrete type or message text. A permanent failure is logged and swallowed, because retrying it would block the processing group's queue over an event that can never succeed.  That leaves a real gap, recorded under «Decisiones pendientes» rather than papered over: a permanently failed createDeposit strands the deposit in PENDING with no way to fail it, since the only path to FAILED demands a registered provider reference and a providerEventId, and neither exists when the charge was never created. Closing it needs a new command on the aggregate.  No tests in this commit, by decision — the checklist item stays open. | 02/09/2026 |
| liquilabshq/vankoo-finance-service | feature/finance-deposit-charge-creation | 79fd5f3 | docs(finance): document the environment variables Stripe needs | Nothing in the repository said what configuration the service needs to talk to Stripe. The five stripe.* placeholders only appeared in application.yaml, and their defaults are empty on purpose, so a missing one does not fail at startup: it fails the first time a deposit is created or a webhook arrives. That is a hard error to place without knowing where to look.  Adds .env.example with every variable the service reads, and a Spanish guide under docs/guides/ covering the Stripe ones: which operation needs which, the exact IllegalStateException each missing value produces, where each value comes from in the dashboard, and the local `stripe listen` flow.  Two things the guide records because they are not evident from the code: STRIPE_ENABLED=false stops outbound calls but does NOT disable inbound webhook verification, since verifyWebhook never consults it; and the webhook signing secret differs between `stripe listen` and a deployed endpoint.  Also drops StripePaymentProperties.failureUrl. It had no binding in application.yaml and no reader anywhere, and Checkout has no failure URL to bind it to — a declined payment is retried inside Checkout and, if it fails for good, arrives as a webhook. Documenting a variable that does nothing would be worse than leaving it undocumented.  .env is added to .gitignore in the same commit: it holds STRIPE_SECRET_KEY, and it was not ignored before. | 02/09/2026 |
| liquilabshq/vankoo-finance-service | develop | 7b7d213 | Merge pull request #19 from liquilabshq/chore/finance-service-eureka-name | chore(finance): drop the vankoo prefix from spring.application.name | 31/08/2026 |
| liquilabshq/vankoo-finance-service | chore/finance-service-eureka-name | 7a178e9 | chore(finance): drop the vankoo prefix from spring.application.name | The name is the Eureka service ID, and everyone else registers without the prefix: iam-service and api-gateway. Finance was the only one carrying it, and it already contradicted itself — application-docker.yaml registers the instance hostname as finance-service, and the documentation gateway-url serves this service under /finance.  Free to change today, not tomorrow: no one resolves this name yet. The gateway has no Finance route (only the commented example block), and the Kafka producer binds by explicit destination, not by application name. Once a lb:// route exists in the gateway, renaming would need both repos to move together.  Verified against real infrastructure: the service registers as FINANCE-SERVICE, vipAddress finance-service, status UP on 8083, and deregisters cleanly on shutdown.  The artifact keeps its full name — pom.xml, the repo and the docs still say vankoo-finance-service, where the prefix distinguishes the organization. | 31/08/2026 |
| liquilabshq/vankoo-finance-service | develop | 33348c2 | Merge pull request #18 from liquilabshq/feature/wallet-rest-endpoints | feat(finance): add read-only REST endpoints for Wallet | 30/08/2026 |
| liquilabshq/vankoo-finance-service | feature/wallet-rest-endpoints | 597b7fe | feat(finance): add read-only REST endpoints for Wallet | GET /v1/accounts/{accountId}/wallets/{currency} (balance) and .../movements (paginated history) — Trello card #44.  WalletProjection is the first append-only Read Model projection in this service: wallet_views upserts one row per wallet (same alreadyApplied guard as deposit_views), but wallet_movements inserts a new row per event and never updates one, so its dedup barrier against Axon's at-least-once redelivery is insertIfAbsent keyed on the event id — the same idiom finance_ops tables already use for a different reason, applied here to a Read Model table for the first time. Both tables are written from the same three events in one transaction, by one class.  WalletCreditedEvent/WalletDebitedEvent carry no accountId, so the projection reads it off the already-loaded balance row. WalletMovementType (the write side's debit reasons) deliberately excludes RECARGA — crediting is its own command, not a debit reason — so the read side gets its own vocabulary, WalletMovementKind, that adds it back for movement history labeling.  getWalletBalance 404s for a wallet that was never opened (Wallet is created lazily on an investor's first deposit per currency); listWalletMovements never does, symmetric with listDeposits — a collection endpoint answers an empty page, not a 404, for an account with nothing yet.  Also fixes a real bug found while testing this: UnsupportedCurrencyException does not extend IllegalArgumentException, so an unsupported currency on POST /v1/deposits (already-merged code) was falling through to a 500 instead of the intended 400. Same fix applied to the new WalletController.  Verified live against real Postgres + Axon Server (open/credit/debit a wallet via CommandGateway directly, since nothing in this environment drives a Deposit to SUCCEEDED yet) that both tables project correctly. | 30/08/2026 |
| liquilabshq/vankoo-finance-service | develop | f853591 | Merge pull request #17 from liquilabshq/feature/deposit-rest-endpoints | feat(finance): add REST endpoints for Deposit | 27/08/2026 |
| liquilabshq/vankoo-finance-service | feature/deposit-rest-endpoints | 1c8c855 | feat(finance): add REST endpoints for Deposit | POST /v1/deposits, GET /v1/deposits/{id} and GET /v1/accounts/{id}/deposits, per the routes already documented in finance-contracts.md and modeled in finance-rest-api-class-diagram.puml.  Building the endpoint required DepositCommandService/Impl first (Card 4's own prerequisite): it did not exist yet, only the query side did. Its Idempotency-Key barrier inserts before dispatching, not after, closing a race where two concurrent requests with the same key could both dispatch InitiateDepositCommand; @Transactional spans the CommandGateway call so a rejected command rolls the reservation back instead of leaving it dangling.  DepositResource extends the diagram with description/cancellationReason/ updatedAt, since DepositSummary already carries them. A replayed Idempotency-Key answers with the deposit's real current status (safe to query — the deposit has existed for a while); a first-time create builds the 202 body from the command's own inputs instead, avoiding a read-before- projected race with DepositProjection's own event processor.  Reconciled finance-application-services-class-diagram.puml, which had drifted out of sync with the shipped DepositQueryService and never matched what this card built for DepositCommandService (no PaymentProvider call, no ApplyProviderDepositUpdateCommand handling — the inbox still dispatches that one directly, unchanged).  Nothing yet reacts to DepositInitiatedEvent to call PaymentProvider, so a deposit created through this endpoint stays PENDING — tracked as a separate Trello card, deliberately out of scope here. | 27/08/2026 |
| liquilabshq/vankoo-finance-service | develop | 562c429 | Merge pull request #16 from liquilabshq/fix/wallet-creditor-eventhandlers-layering | refactor(finance): move WalletCreditor to application/internal/eventhandlers | 25/08/2026 |
| liquilabshq/vankoo-finance-service | fix/wallet-creditor-eventhandlers-layering | 2ebab9f | refactor(finance): move WalletCreditor to application/internal/eventhandlers | It reacts to DepositSucceededEvent, one of our own events, and writes finance_ops.wallet_credited_deposits for idempotency — exactly the shape Card 8's new eventhandlers rule (AGENTS.md) describes. It stayed in commandservices when the rule was written; this closes that gap.  No behavior change: package move only, same class, same tests. | 25/08/2026 |
| liquilabshq/vankoo-finance-service | develop | 614fb9c | Merge pull request #15 from liquilabshq/feature/finance-integrations-events | Feature/finance integrations events | 22/08/2026 |
